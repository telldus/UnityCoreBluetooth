...
@_cdecl("ucb_peripheral_getUUID")
public func ucb_peripheral_getUUID(_ peripheral: UnsafePointer<CBPeripheral>) -> UnsafePointer<CChar>? {
    let peripheral = Unmanaged<CBPeripheral>.fromOpaque(peripheral).takeUnretainedValue()
    let nsStr = peripheral.identifier.uuidString as NSString
    let uuidString = nsStr.utf8String!
    let len = strlen(uuidString) + 1
    let ptr = UnsafeMutablePointer<CChar>.allocate(capacity: len)
    ptr.initialize(from: uuidString, count: len)
    return UnsafePointer(ptr)
}
